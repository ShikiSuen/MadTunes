...
extension ThisDevice {
  ...
  public static var deviceRAMInsufficientThresholdAsGiB: Int {
    switch OS.type {
    case .macOS: 16
    case .iPadOS: 8 // <- Only for MadTunes.
    case .iPhoneOS: 8
    case .watchOS: 1
    case .tvOS: 4
    }
...
/// 用於快取 identifier4Vendor 的執行緒安全容器。
...
public enum OS: Int, Sendable {
  ...
  /// iOS 18.0 ~ 18.3 有關於畫面底部的 Bottom Toolbar 的故障，需要單獨應對。
  ...
  public static let isBetaOSBeforeFirstMajorPublicRelease: Bool = {
    ...
    // 檢查是否是大版本的 X.0.0（即 major 版本任意，minor 和 patch 都是 0）
    // 這涵蓋了 Dev Beta 和 Public Beta 的早期階段
    if systemVersion.minorVersion == 0, systemVersion.patchVersion == 0 {
      // 進一步確認是否是 Beta（通過檢查 build 號或額外標記）
...
extension ThisDevice {
  ...
  /// 檢測熒幕解析度是否為 iPhone 5 / 5c / 5s / SE Gen1 / iPod Touch 7th Gen 的最大熒幕解析度。
  /// 如果是 iPhone SE2 / SE3 / 6 / 7 / 8 且開啟了熒幕放大模式的話，也會用到這個解析度。
  /// 不考慮 4:3 熒幕的機種（iPhone 4s 為止的機種）。
...
enum OSBuild {
  ...
  /// 判斷當前系統是否是 Beta 版本 (基於 Build Number 結尾是否為字母)
  fileprivate static func isUnstableBetaBuild() -> Bool {
    // 1. 獲取系統 Build 版本號 (例如: "21A5277g")
    guard let buildString = getSystemBuildString() else { return false }

    // 2. 核心判斷邏輯
    // 正式版的 Build Number 通常以數字結尾 (如 "21A329")
    // Beta 版通常以字母結尾 (如 "21A5277g")
    // 我們檢查最後一個字符是否是字母
