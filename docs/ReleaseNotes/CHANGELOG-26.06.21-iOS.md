...
- **并发故障修复**：修复了一些并发问题（借助 Swift 6.4 beta 编译器的增强检测能力才得以发现，Swift 6.3 工具链（截至 Xcode 26.5）无法侦测到这些问题）。
...
- **並行故障修正**：修正了一些並行問題（借助 Swift 6.4 beta 編譯器的增強偵測能力才得以發現，Swift 6.3 工具鏈（截至 Xcode 26.5）無法偵測到這些問題）。
